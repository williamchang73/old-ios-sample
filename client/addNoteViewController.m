...
- (void)viewDidLoad
{
    if ([authToken isEqualToString:@"your developer token" ]) {
        
        // Alerting the user that the note was created
        UIAlertView *alertDone = [[UIAlertView alloc] initWithTitle: @"Evernote" message: @"Please set your developer token in Evernote.m" delegate: self cancelButtonTitle: @"Ok" otherButtonTitles: nil];
...
-(IBAction)sendNoteEvernote:(id)sender{
    ...
    if ([titleNote.text length] == 0) {
        NSString * errorMessage = [NSString stringWithFormat:@"Please set a note title"];
